...
# ==============================================================================
# SUBPARTE 3: NÚCLEO MULTIMEDIA, COLORES E INTERRUPTOR DINÁMICO DE TRADUCCIÓN
# ==============================================================================
def crear_boton_practica(texto_en_ingles, texto_en_espanol, id_unico, url_imagen=None):
    """
    Procesa imágenes de forma segura en el servidor, renderiza el mapa lingüístico,
    inyecta el motor de voz de EE. UU. (Hombre/Mujer) y añade un interruptor nativo
    para ocultar o mostrar la traducción al español.
    """
    # 1. Descargador seguro de imágenes para evitar bloqueos de red
    if url_imagen:
        try:
    ...
    # 2. Desplegar texto en Inglés con su mapa de colores
    st.markdown(f"📖 **English:** {texto_en_ingles}", unsafe_allow_html=True)
    
    # 3. INTERRUPTOR DIDÁCTICO: Permite ocultar y aparecer el español a voluntad
    # Cambia su etiqueta dinámicamente según el estado del estudiante
    toggle_key = f"toggle_esp_{id_unico}"
    ver_traduccion = st.toggle(
        "👁️ Mostrar traducción al español" if st.session_state.get(toggle_key, False) else "🙈 Traducción oculta (Practicar)", 
        key=toggle_key
    )
    ...
    # 4. Limpiar etiquetas HTML para que el lector de voz no las pronuncie
    texto_plano = re.sub(r'<[^>]+>', '', texto_en_ingles)
    texto_escapado = texto_plano.replace("'", "\\'").replace('"', '\\"')
    
    # Clasificación estricta de género según el identificador de la frase
...
# ==============================================================================
# SUBPARTE 5: SECCIÓN CORE I (OBJECTIVE Y REPORTES ESCRITOS CON APERTURA DE INTERROGACIÓN)
# ==============================================================================
...
# ==============================================================================
# SUBPARTE 6: SECCIÓN CORE II (SPOKEN ENGLISH, AUDITORÍAS Y SIGLAS FORMATEADAS)
# ==============================================================================
...
# ==============================================================================
# SUBPARTE 7: SIMULADOR DE FEEDBACK CASUAL CON SUPERVISOR COMPLETAMENTE CORREGIDO
# ==============================================================================
